...
def train():
    ...
    if algorithm_name not in algorithm_name_list:
        print(
            "\033[92m"
            + f"algorithm_name: {algorithm_name} not in list {algorithm_name_list}"
            + "\033[0m"
        )
    ...
    if sample_tool_type is None:
        ...
        result_code, result_str = python_exec_shell(
            f"sh tools/change_sample_server.sh {sample_tool_type}"
        )
        if result_code != 0:
            raise ValueError(
                f"Execution error! Please check the error detail: {result_str}"
            )
...
def check(proc: Process):
    if not proc.is_alive():
        ...
        print(
            "\033[1;31m"
            + f"{proc.name} is not alive, please check error log"
            + "\033[0m"
        )
        python_exec_shell(f"sh tools/stop.sh all")
